---
name: review-cycle
description: Multi-stage code review pipeline. Use when the developer wants a thorough, structured review of code changes with adjudication, planning, and automated fixes. Invoke with /review-cycle.
...
produced `.review-cycle/review-findings.md`. Read the file and confirm the
finding count to the developer.

### Stage 2 — Adjudicate

Pass to the **Adjudication Lead**:
- The contents of `.review-cycle/review-findings.md`
- The original problem being solved

The Adjudication Lead will spawn investigator sub-agents, compile results into
`.review-cycle/adjudication-report.md`, then go idle briefly.
...
`.review-cycle/approved-findings.md` and goes idle.
...
- The contents of `.review-cycle/approved-findings.md`
...
- The contents of `.review-cycle/fix-plan.md`

The Execution Lead will assign fixes to sub-agents and track progress by
updating `.review-cycle/fix-plan.md` with completion status. When all fixes
...
All artifacts are written to `.review-cycle/` in the working directory.
...
If the session is interrupted and the developer re-invokes `/review-cycle`,
check for existing artifacts in `.review-cycle/`:
